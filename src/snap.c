...
void snap_update(struct planar_server *server, struct planar_toplevel *moving,
    ...
    wl_list_for_each(other, &server->active_workspace->toplevels, link) {
        ...
        double o_right = other->logical_x + other->decoration->width;
        double o_center_x = other->logical_x + other->decoration->width / 2.0;
        double o_top = other->logical_y;
        double o_bottom = other->logical_y + other->decoration->height;
        double o_center_y = other->logical_y + other->decoration->height / 2.0;
